...
```{r}
tbl_perf %>%
  dplyr::select(scenario, UDdepen, n, p, epslvl, propzi, struczero, revzi, model, penalty, RMSPE.est) %>%
  filter(scenario %in% "A" & penalty %in% c("combined", "-") & UDdepen == "U=D" & struczero == 0.33 & revzi == TRUE & model != "oracle") %>%
  mutate(n = to_factor(n)) %>%
  arrange(model, penalty) %>%
  pivot_wider(id_cols = c(propzi, n, p, epslvl), values_from = RMSPE.est, names_from = model) %>%
  nested_loop_plot(x = "n", steps = "epslvl", grid_cols ="propzi" ,
