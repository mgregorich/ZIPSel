...
ggsave(file = here::here("output", "example","fig_dist_zi_mosaiques.tiff"), plot = p1,
       width = 1.1*100, height = 1*100, units = "mm", dpi = 350, compression = "lzw") 
```

```{r}
data_cor <- cor(data_peptides)
pheatmap::pheatmap(data_cor)
```
...
p2 <- data.frame("intensity" = ximp[, ind]) %>% 
  ...
  theme_light() + 
  theme(plot.title = element_text(hjust=0.5)) +
  ggtitle(expression(paste("Transformed data ",tilde(X))))
...
penalty <- c(rep(c("combined", "component"), 5)[-2])
nmod <- length(model)
...
for(i in 1:ncv){
  ...
  fit.ridge.cb <- perform_penreg(obj.train, family="gaussian", alpha1 = 0, cv=ncv, R=nR, penalty = "combined", 
                                 split_vars = TRUE)
  fit.ridge.cp <- perform_penreg(obj.train, family = "gaussian", alpha1 = 0, cv = ncv, R = nR, penalty  =  
                                   "component", pflist = pflist, split_vars = TRUE)
  fit.lridge.cb <- perform_lridge(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                  split_vars = TRUE)
  fit.lridge.cp <- perform_lridge(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "component", 
                                  pflist = pflist, split_vars = TRUE)
  fit.rlasso.cb <- perform_rlasso(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                  split_vars = TRUE)
  fit.rlasso.cp <- perform_rlasso(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "component", 
                                  pflist = pflist, split_vars = TRUE)
  fit.rgarrote.cb <- perform_rgarrote(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                      split_vars = TRUE)
  fit.rgarrote.cp <- perform_rgarrote(obj.train, family = "gaussian", cv = ncv, R = nR, penalty = "component", 
                                      pflist = pflist, split_vars = TRUE)
...
list_model$fit.ridge.cb <- perform_penreg(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                          split_vars = TRUE)
list_model$fit.ridge.cp <- perform_penreg(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "component",
                                          pflist = pflist, split_vars = TRUE)
list_model$fit.lridge.cb <- perform_lridge(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                           split_vars = TRUE)
list_model$fit.lridge.cp <- perform_lridge(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "component", 
                                           pflist = pflist, split_vars = TRUE)
list_model$fit.rlasso.cb <- perform_rlasso(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                           split_vars = TRUE)
list_model$fit.rlasso.cp <- perform_rlasso(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "component",
                                           pflist = pflist, split_vars = TRUE)
list_model$fit.rgarrote.cb <- perform_rlasso(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "combined", 
                                             split_vars = TRUE)
list_model$fit.rgarrote.cp <- perform_rlasso(data.obj = data.obj, family = "gaussian", cv = ncv, R = nR, penalty = "component", 
                                             pflist = pflist, split_vars = TRUE)
...
tbl.coef.rlasso.cb <- data.frame("peptide" = str_remove_all(names(list_coef$rlasso.cb)[2:(p+1)], "x.|d."), 
                                 "U" = list_coef$rlasso.cb[2:(p+1)], 
                                 "D" = list_coef$rlasso.cb[(p+2):(2*p+1)]) 
...
tbl.coef.rgarrote.cb <- data.frame("peptide" = str_remove_all(names(list_coef$rgarrote.cb)[2:(p+1)], "u.|d."), 
                                   "U" = list_coef$rgarrote.cb[2:(p+1)],
                                   "D" = list_coef$rgarrote.cb[(p+2):(2*p+1)]) 
...
tbl_zi_coef_D <- cbind(data_zi, tbl.coef.ridge.cb$D,
                     tbl.coef.lridge.cb$D, tbl.coef.rlasso.cb$D,
                     tbl.coef.rgarrote.cb$D) %>%
  data.frame() %>%
...
tbl_zi_coef_X <- cbind(data_zi, tbl.coef.ridge.cb$U,
                     tbl.coef.lridge.cb$U, tbl.coef.rlasso.cb$U,
                     tbl.coef.rgarrote.cb$U) %>%
  data.frame() %>%
  `colnames<-`(c("variable", "zi", "ridge", "lridge",
                 "rlasso", "rgarrote")) %>%
  mutate(component="U")%>%
...
```{r}
pcal_lasso <- plot_calibration(pred = data.obj$pred.lasso.cb, obs = data_mos$log2eGFR, fig.title = "penalty: combined")
pcal_lasso
```
