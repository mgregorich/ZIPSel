...
```{r}
# Data generation
data_sim <- data_generation(dsgn = dsgn, population = scn$population, n = scn$n,
...
-   **population A**: Only peptides in group 1 have an effect on the outcome variable $y$. The effect is linearly decreasing for the true predictors in the continuous component $X_{struc}$, while it is permuted for the true predictors in the binary component $D_{struc}$, otherwise it is 0.
-   **population B**: In each group, 5 peptides which are spaced equally with regard to the index are true predictors. The effect again linearly decreases for the true predictors in the continuous component $X_{struc}$ and is randomly permuted for the true predictors in the binary component $D_{struc}$, otherwise 0.
...
true_beta %>%
  ...
  {if(scn$population=="B")geom_point()} +
  geom_line() +
...
data.val <- data_generation(dsgn = dsgn, population = scn$population, n = 100000,
                            p=scn$p, beta_max = scn$beta_max, 
                            a = scn$a, epsstd = scn$epsstd, xmean = scn$xmean, xstd = scn$xstd,
                            propzi = scn$propzi, revzi = scn$revzi, 
                            struczero = scn$struczero)
...
```{r}
# --- Lasso-ridge 
data.obj3 <- generate_dataobj_reduced(y = data_sim$data_ana$y, 
                                 x = data_sim$data_ana$X,
                             clinical = NULL, logtransform = FALSE, n=200)
...
```{r}
pcal_oracle <- plot_calibration(pred = tbl_pred$pred.oracle, obs = tbl_pred$y, fig.title = "oracle")
