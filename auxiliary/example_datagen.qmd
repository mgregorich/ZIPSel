...
p_cor_theo <- ggplot(data = melted_cormat, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1,1), space = "Lab", 
                       name="") +
...
#| fig.width: 8
#| fig.height: 4
corsim <- as.matrix(cor(sim_data))
colnames(sim_data) <- rownames(sim_data) <- NULL
melted_cormat <- reshape2::melt(round(corsim,4))

p_cor_emp <- ggplot(data = melted_cormat, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(-1,1), space = "Lab", 
                       name="") +
  ...
  scale_y_continuous("Candidate variables", expand=c(0,0)) +
  theme_bw() 
p_cor_theo + p_cor_emp + plot_annotation(tag_levels = 'A') +  plot_layout(guides = "collect")
ggsave(file = here::here("output", "figures", "fig_correlation_structure.tiff"), width=3*100, height=1.5*100, units="mm", dpi=350, compression = "lzw")
```
