...
data.frame("Parameter"=rownames(t(scn[,1:13])), "Value"=t(scn[,1:13])) %>% rename("Value"=paste0("X", scn_nr)) %>% gt()
```
...
```{r}
# Data generation
data_sim <- data_generation(dsgn = dsgn, scenario = scn$scenario, n = scn$n,
...
-   **Scenario A**: Only peptides in group 1 have an effect on the outcome variable $y$. The effect is linearly decreasing for the true predictors in the continuous component $X_{struc}$, while it is permuted for the true predictors in the binary component $D_{struc}$, otherwise it is 0.
-   **Scenario B**: In each group, 5 peptides which are spaced equally with regard to the index are true predictors. The effect again linearly decreases for the true predictors in the continuous component $X_{struc}$ and is randomly permuted for the true predictors in the binary component $D_{struc}$, otherwise 0.
...
p_Dstrc <- ggplot(data = melt_Dstr, aes(x=Var2, y=Var1, fill=value_rev)) +
 geom_tile()+
...
p_Dsamp <- ggplot(data = melt_Dsamp, aes(x=Var2, y=Var1, fill=value_rev)) +
 geom_tile()+
 scale_fill_gradient2(low = "white", high = "red",
   limit = c(0,1), name="") +
  theme_bw()+ 
...
```{r}
#| fig-cap: "Distribution of prepared peptide intensities excluding zero values"
p1 <- data.frame("intensity" = c(data_sim$data_ana$X)) %>% 
  ggplot(aes(x = intensity)) + 
  geom_histogram(col = "black", fill = "grey89", bins = 30) +
  scale_x_continuous("Intensity") +
  scale_y_continuous("Count") +
...
p2 <- data.frame("intensity" = c(data.obj$x)) %>% 
  ...
  theme_light() + 
  theme(plot.title = element_text(hjust=0.5)) +
  ggtitle("Conventional approach: X")
p3 <- data.frame("intensity" = c(data.obj$u)) %>% 
  ...
  theme_light() + 
  theme(plot.title = element_text(hjust=0.5)) +
  ggtitle("Data split: continuous U")
...
data.val <- data_generation(dsgn = dsgn, scenario = scn$scenario, n = 100000,
                            p=scn$p, beta_max = scn$beta_max, 
                            a = scn$a, epsstd = scn$epsstd, xmean = scn$xmean, xstd = scn$xstd,
                            propzi = scn$propzi, revzi = scn$revzi, 
                            struczero = scn$struczero)
data.obj.val <- generate_dataobj(y = data.val$data_ana$y, 
                                 x = data.val$data_ana$X,
                             clinical = NULL, logtransform = FALSE)
```
...
# X <- cbind(data_sim$data_gen$X_struc, data_sim$data_gen$D_struc)
beta_true <- c(true_beta$beta_X, true_beta$beta_D)
Xtrue <- X[,which(beta_true!=0)]
fit.oracle <- lm(data.obj$y~Xtrue)
...
# tbl_pred$pred.oracle <-  beta_oracle[1] + c(cbind(data.val$data_gen$X_struc, data.val$data_gen$D_struc) %*% beta_oracle[-1])
tbl_perf[1, 3:6] <- eval_performance(pred = tbl_pred$pred.oracle, 
                                     obs = tbl_pred$y)
...
tbl_perf[2, 3:6] <- eval_performance(pred = tbl_pred$pred.lasso, 
                                     obs = tbl_pred$y)
...
tbl_perf[3, 3:6] <- eval_performance(pred = tbl_pred$pred.ridge, obs = tbl_pred$y)
tbl_perf[3, "df"] <- fit.ridge$dfvars
tbl_perf[3, "extime"] <- fit.ridge$extime
...
tbl_perf[4, 3:6] <- eval_performance(pred = tbl_pred$pred.lridge, obs = tbl_pred$y)
tbl_perf[4, "df"] <- fit.lridge$dfvars
tbl_perf[4, "extime"] <- fit.lridge$extime
...
tbl_perf[5, 3:6] <- eval_performance(pred = tbl_pred$pred.rlasso, obs = tbl_pred$y)
tbl_perf[5, "df"] <- fit.rlasso$dfvars
tbl_perf[5, "extime"] <- fit.rlasso$extime
...
#| fig-cap: "Component-specific coefficients of the ridge-lassi regression model by D and U, respectively." 
...
tbl_perf[6, 3:6] <- eval_performance(pred = tbl_pred$pred.rgarrote, obs = tbl_pred$y)
tbl_perf[6, "df"] <- fit.rgarrote$dfvars
tbl_perf[6, "extime"] <- fit.rgarrote$extime
...
```{r}
tbl_allsel %>% mutate_at(3:6, round, 2) %>% gt()
```
...
tbl_perf %>%
  data.frame() %>%
  mutate(model=fct_relevel(model, c( "oracle","lasso", "ridge", "lasso-ridge",
